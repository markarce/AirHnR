...
module.exports.getGoogleResults = (q) => {
  return new Promise((resolve, reject) => {
    request(`https://maps.googleapis.com/maps/api/place/autocomplete/json?input=${q}&types=geocode&key=AIzaSyAksWYj-KkFtTqnuYNI9vNZToFFyixc65w`, (error, response, body) => {
      if (error) { return reject(error) }
      resolve(JSON.parse(body).predictions.map(obj => { return { desc: obj.description, sec: obj.structured_formatting.secondary_text } }));
    });
  });
};